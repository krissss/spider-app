...
class Migration extends \yii\db\Migration
{
    ...
    public function commonColumns($include = [])
    {
        $arr = [
            ...
            'is_deleted' => $this->boolean()->notNull()->defaultValue(false)->comment('是否被删除'),
            ...
            'created_type' => $this->integer()->notNull()->defaultValue(0)->comment('创建人类型'),
            'version' => $this->bigInteger()->notNull()->defaultValue(0)->comment('版本号'),
        ];
        $returnArr = [];
        if ($include) {
     ...
     * @param null $update
     */
    public function addForeignKeyCustom($table, $columns, $refTable, $nameSuffix = '', $refColumns = 'id', $delete = null, $update = null)
    {
        $name = 'fk-' . $table . '-' . $refTable . $nameSuffix;
